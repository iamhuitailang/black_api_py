const LineChart = {
    ...
    init() {
        ...
        this.tooltip = document.getElementById('lineChartTooltip');
        ...
        ChartEngine.register('lineChart', this);
    ...
    render() {
        const data = DataStore.getLineChartData();
        ...
        const result = ChartEngine.drawLineChart(this.ctx, {
            x: padding.left,
            y: padding.top,
            width: chartWidth,
            height: chartHeight,
    ...
    handleClick(e) {
        if (this.hoveredPoint) {
            DrillService.show('lineChart', this.hoveredPoint.index);
        }
    },
    ...
    showTooltip(e, point) {
        ...
        pointsAtSameIndex.forEach(p => {
            html += `<div class="tooltip-value" style="color: ${p.color}">
                ${p.datasetLabel}: ${DateUtils.formatCurrency(p.value)}
            </div>`;
        });
