const SalePage = {
    ...
    async loadCustomers() {
        try {
            const result = await ContactService.getAllCustomers();
            if (result.code === 0) {
    ...
    async loadData() {
        try {
            ...
            if (this.filterCustomer) {
                params.customer_id = this.filterCustomer;
            }
            if (this.startDate) {
    ...
    renderContent() {
        ...
        const customerOptions = this.customers.map(c => 
            `<option value="${c.id}" ${this.filterCustomer == c.id ? 'selected' : ''}>${c.name}</option>`
        ).join('');
            ...
            <tr>
                ...
                <td>
                    <div class="table-actions">
                        <button class="btn btn-secondary btn-sm" onclick="SalePage.edit(${item.id})">编辑</button>
                        <button class="btn btn-danger btn-sm" onclick="SalePage.confirmDelete(${item.id})">删除</button>
                    </div>
                </td>
            </tr>
        `).join('') : `
            <tr>
                <td colspan="10">
                    <div class="empty-state">
                        <div class="icon">📤</div>
                        <p>暂无销售记录</p>
                        <button class="btn btn-primary mt-2" onclick="SalePage.add()">
                            新增销售
                        </button>
                    </div>
                </td>
        ...
        const paginationHtml = Layout.renderPagination(this.data, 'SalePage.goToPage');
        
        const content = `
            <div class="page-header">
                <h1 class="page-title">销售管理</h1>
                <p class="page-subtitle">管理所有销售记录</p>
            </div>
                ...
                <div class="card-header">
                    <div class="toolbar">
                        <div class="toolbar-left">
                            <select class="form-control" style="width: auto;" onchange="SalePage.changeVariety(this)">
                                <option value="">全部品种</option>
                                ${varietyOptions}
                            </select>
                            <select class="form-control" style="width: auto;" onchange="SalePage.changeCustomer(this)">
                                <option value="">全部客户</option>
                                ${customerOptions}
                            </select>
                            <input type="date" class="form-control" style="width: auto;" value="${this.startDate}" 
                                placeholder="开始日期" onchange="SalePage.changeStartDate(this)">
                            <input type="date" class="form-control" style="width: auto;" value="${this.endDate}"
                                placeholder="结束日期" onchange="SalePage.changeEndDate(this)">
                        </div>
                        <div class="toolbar-right">
                            <div class="search-box">
                                <span class="search-icon">🔍</span>
                                <input type="text" class="form-control" placeholder="搜索品种/客户/销售地..." 
                                    value="${this.searchKeyword}"
                                    onkeyup="SalePage.handleSearch(event)">
                            </div>
                            <button class="btn btn-success" onclick="SalePage.add()">
                                ➕ 新增销售
                            </button>
                        </div>
                    </div>
    ...
    changeCustomer(select) {
        this.filterCustomer = select.value;
        this.data.page = 1;
        this.loadData().then(() => this.renderContent());
    },
    ...
    showForm() {
        ...
        const title = this.editMode ? '编辑销售记录' : '新增销售记录';
        ...
        const customerOptions = this.customers.map(c => 
            `<option value="${c.id}" ${item.customer_id == c.id ? 'selected' : ''}>${c.name}</option>`
        ).join('');
            ...
            <form id="saleForm">
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="unit_price" class="form-control" step="0.01" min="0"
                                value="${item.unit_price || ''}" placeholder="请输入单价" oninput="SalePage.calcTotal()">
                        </div>
                        <div class="form-group">
                            <label class="form-label">
                            ...
                            <input type="number" name="quantity" class="form-control" min="1"
                                value="${item.quantity || ''}" placeholder="请输入数量" oninput="SalePage.calcTotal()">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label class="form-label">销售地</label>
                            <input type="text" name="sale_location" class="form-control"
                                value="${item.sale_location || ''}" placeholder="请输入销售地">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="sale_date" class="form-control"
                            value="${item.sale_date || Layout.getTodayString()}">
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-success" id="saveSaleBtn">保存</button>
                </div>
            </form>
        `;
    ...
    async saveForm(form) {
        ...
        try {
            ...
            if (this.editMode && this.editItem) {
                ...
                result = await SaleService.update(data);
            } else {
                result = await SaleService.add(data);
            }
