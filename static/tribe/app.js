    ...
    setup() {
        ...
        async function doTrade() {
            const r = await api('/trade', 'POST', {
                ...
                foreign_tribe_id: currentTradeTribe.value.id,
                ...
                <div class="resource-bar">
                    <div v-for="(info, key) in RESOURCE_INFO" :key="key" class="resource-item">
                        ...
                        <span class="resource-delta"
                              :class="{
                                  'delta-positive': (gameState.production?.[key] || 0) > 0 && (key !== 'food'),
                                  'delta-negative': key === 'food' && (gameState.production?.food || 0) - (gameState.food_consumption || 0) < 0
                              }">
                            <span v-if="key === 'food'">({{ (gameState.production?.food || 0) > 0 ? '+' : '' }}{{ (gameState.production?.food || 0) }}/-{{ gameState.food_consumption || 0 }})</span>
                            <span v-else-if="gameState.production?.[key] > 0">(+{{ gameState.production[key] }})</span>
                        </span>
                    </div>
                        ...
                        <div v-if="activeTab === 'panorama'">
                            ...
                            <div class="building-grid">
                                <div v-for="b in gameState.buildings" :key="b.id"
                                    ...
                                    <div>
                                        <div class="building-name">{{ gameState.building_names?.[b.building_type] || b.building_type }}</div>
                        ...
                        <div v-else-if="activeTab === 'tech'">
                            ...
                            <div class="branch-legend">
                                <div class="branch-legend-item">
                                    <div class="branch-dot" style="background: var(--branch-root);"></div>核心科技
                                </div>
                                <div class="branch-legend-item">
                                    <div class="branch-dot" style="background: var(--branch-military);"></div>军事分支
                                </div>
                                <div class="branch-legend-item">
                                    <div class="branch-dot" style="background: var(--branch-economy);"></div>经济分支
                                </div>
                                <div class="branch-legend-item">
                                    <div class="branch-dot" style="background: var(--branch-civic);"></div>文化分支
                                </div>
                                <div class="branch-legend-item" style="color: var(--danger); margin-left: auto;">
                                    ═══ 红色虚线 = 互斥关系（只能选一个）
                                </div>
                            </div>
                            ...
                            <div class="stats-grid" style="grid-template-columns: repeat(4, 1fr);">
                                <div class="stat-card">
                                    <div class="stat-label">当前知识</div>
                                    <div class="stat-value">📚 {{ gameState.tribe.knowledge }}</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">已研究</div>
                                    <div class="stat-value" style="color: var(--success);">{{ gameState.researched_ids.length }}项</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">可研究</div>
                                    <div class="stat-value" style="color: var(--accent);">{{ gameState.available_techs.length }}项</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">晋级要求</div>
                                    <div class="stat-value" style="color: var(--warning);">
                                        {{ gameState.era_advance_requirements || '-' }}
                                    </div>
                                </div>
                            </div>

                            <div class="tech-tree-container">
                                <svg class="tech-tree-svg" width="950" height="1150" viewBox="0 0 950 1150"
                                     v-html="getTechSvgPath()"></svg>
                            </div>
                        </div>
                            ...
                            <div class="stats-grid" style="grid-template-columns: repeat(auto-fill, minmax(150px, 1fr));">
                                <div v-for="(name, job) in JOB_NAMES" :key="job" class="stat-card">
                                    ...
                                    <div class="stat-value" style="font-size: 1.3rem;">
                                        {{ gameState.tribespeople.filter(p => p.job === job).length }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="diplomacy-list">
                                <div v-for="ft in gameState.diplomacy" :key="ft.foreign_tribe_id" class="diplomacy-card">
                                    <div class="diplomacy-header">
                                        <div>
                                            ...
                                            <div style="font-size: 0.8rem; color: var(--text-dim); margin-top: 2px;">
                                                <span class="era-badge" :class="ERA_STYLE_CLASS[ft.foreign_era]" style="font-size: 0.7rem; margin-right: 6px;">
                                                    {{ ERA_NAMES[ft.foreign_era] }}
                                                </span>
                                                特产: {{ RESOURCE_INFO[ft.specialty_resource]?.icon }} {{ RESOURCE_INFO[ft.specialty_resource]?.name || ft.specialty_resource }}
                                            </div>
                                        </div>
                                        <span class="relation-badge"
                                              :class="'relation-' + (ft.relation === 'war' ? 'war' : ft.attitude === 'hostile' ? 'hostile' : ft.attitude === 'friendly' ? 'friendly' : 'neutral')">
                                    ...
                                    <div class="strength-bar">
                                        <div class="strength-fill" :style="{ width: Math.min(100, ft.strength / 3) + '%' }"></div>
                                    </div>
                                    <div style="font-size: 0.8rem; color: var(--text-dim); margin-top: 8px;">
                                        {{ ft.attitude === 'hostile' ? '🏹 敌方部落' : ft.attitude === 'friendly' ? '🕊️ 友好邻邦' : '🤝 中立部落' }}
                                        <span v-if="ft.trade_cooldown > 0" style="margin-left: 10px; color: var(--warning);">
                                            ⏳ 贸易冷却: {{ ft.trade_cooldown }}回合
                                        </span>
                                    </div>
                                    <div class="diplomacy-actions">
                                        <button class="btn-diplomacy" style="background: var(--branch-economy);"
                                                :disabled="ft.relation === 'war' || ft.trade_cooldown > 0 || !ft.trade_available"
                                                @click="openTradeModal(ft)">
                                            💰 贸易
                                        </button>
                                        <button v-if="ft.relation !== 'war'" class="btn-diplomacy btn-danger"
                                                @click="declareWar(ft.foreign_tribe_id)">
                                            ⚔️ 宣战
                                        </button>
                                        <button v-if="ft.relation === 'war'" class="btn-diplomacy" style="background: var(--danger);"
                                                @click="doBattle(ft.foreign_tribe_id)">
                                            🎯 发起战斗
                                        </button>
                                        <button v-if="ft.relation === 'war'" class="btn-diplomacy" style="background: var(--warning); color: #333;"
                                                @click="makePeace(ft.foreign_tribe_id)">
                                            🕊️ 求和
                                        </button>
                                    </div>
                                    <div v-if="battleResult && battleResult.foreign_tribe_id === ft.foreign_tribe_id" class="battle-log">
                                        <div :class="['battle-result', battleResult.result]">
                                            {{ battleResult.result === 'victory' ? '🎉 大捷！' : battleResult.result === 'defeat' ? '💀 战败...' : '⚔️ 僵持战' }}
                                        </div>
                                        <div style="font-size: 0.85rem; line-height: 1.8;">
                                            <div>我方伤亡: <span style="color: var(--danger);">{{ battleResult.losses }}</span>人</div>
                                            <div v-if="battleResult.loot && battleResult.result === 'victory'">
                                                战利品: 🍖{{ battleResult.loot.food }} 🪵{{ battleResult.loot.wood }} ⚙️{{ battleResult.loot.metal }}
                                            </div>
                                            <div v-if="battleResult.penalty">
                                                损失: 🍖{{ battleResult.penalty.food }} 🪵{{ battleResult.penalty.wood }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div v-else-if="activeTab === 'resources'">
                            ...
                            <div class="stats-grid">
                                <div class="stat-card">
                                    ...
                                    <div class="stat-value">{{ gameState.tribe.food }}</div>
                                    <div class="stat-meta">
                                        产出: +{{ gameState.production?.food || 0 }} / 消耗: -{{ gameState.food_consumption || 0 }}
                                    </div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">🪵 木材</div>
                                    <div class="stat-value">{{ gameState.tribe.wood }}</div>
                                    <div class="stat-meta">每回合: +{{ gameState.production?.wood || 0 }}</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">🪨 石料</div>
                                    <div class="stat-value">{{ gameState.tribe.stone }}</div>
                                    <div class="stat-meta">每回合: +{{ gameState.production?.stone || 0 }}</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">⚙️ 金属</div>
                                    <div class="stat-value">{{ gameState.tribe.metal }}</div>
                                    <div class="stat-meta">每回合: +{{ gameState.production?.metal || 0 }}</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">📚 知识</div>
                                    <div class="stat-value">{{ gameState.tribe.knowledge }}</div>
                                    <div class="stat-meta">每回合: +{{ gameState.production?.knowledge || 0 }}</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">👥 人口</div>
                                    <div class="stat-value">{{ gameState.tribe.population }} / {{ gameState.tribe.max_population }}</div>
                                    <div class="stat-meta">士气: {{ gameState.tribe.morale }}%</div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">⚔️ 军事</div>
                                    <div class="stat-value" style="color: var(--danger);">{{ gameState.military_strength }}</div>
                                    <div class="stat-meta">
                                        士兵: {{ gameState.tribespeople.filter(p => p.job === 'soldier').length }}人
                                    </div>
                                </div>
                                <div class="stat-card">
                                    <div class="stat-label">📅 季节</div>
                                    <div class="stat-value">{{ SEASON_INFO[gameState.tribe.season].icon }} {{ SEASON_INFO[gameState.tribe.season].name }}</div>
                                    <div class="stat-meta">第{{ gameState.tribe.year }}年 · 回合{{ gameState.tribe.turn }}</div>
                                </div>
                            </div>
                            ...
                            <div style="margin-top: 30px;">
                                ...
                                <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                    <span v-for="techId in gameState.researched_ids" :key="techId"
                                          style="background: var(--bg-card); padding: 6px 12px; border-radius: 6px; font-size: 0.85rem;">
                                        ✅ {{ gameState.tech_names?.[techId] || techId }}
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            <div v-if="showBuildModal" class="modal-overlay" @click.self="showBuildModal = false">
                <div class="modal">
                    ...
                    <div class="modal-body">
                        <div v-if="gameState.available_buildings && gameState.available_buildings.length > 0">
                            <div v-for="b in gameState.available_buildings" :key="b.building_type"
                                ...
                                <div class="build-option-info">
                                    ...
                                    <p>{{ b.description }}</p>
                                    <p v-for="(v, k) in b.effects" :key="k" style="font-size: 0.75rem; color: var(--success); margin-top: 2px;">
                                        + {{ k }}: {{ v }}
                                    </p>
                                </div>
                                <div class="build-option-cost">
                                    <div v-for="(amt, r) in b.cost" :key="r"
                                         :class="['cost-item', { insufficient: amt > 0 && (gameState.tribe[r] || 0) < amt }]">
                        ...
                        <div v-else class="empty-state">
                            <div class="empty-icon">🔒</div>
                            <div>暂无可建造的建筑，研究更多科技以解锁</div>
                        </div>
                    </div>
                    <div class="modal-actions">
                        <button class="btn btn-secondary" @click="showBuildModal = false">关闭</button>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showTechModal && currentTechInfo" class="modal-overlay" @click.self="showTechModal = false">
                <div class="modal">
                    <h3 class="modal-title">
                        ...
                        <span class="era-badge" :class="ERA_STYLE_CLASS[currentTechInfo.era]" style="margin-left: 10px;">
                            {{ ERA_NAMES[currentTechInfo.era] }}
                        </span>
                        <span class="relation-badge" style="margin-left: 8px; background: var(--bg-card); color: var(--text-dim); font-weight: normal;">
                            {{ BRANCH_NAMES[currentTechInfo.position.branch] }}分支
                        </span>
                    </h3>
                    <div class="modal-body">
                        <p style="line-height: 1.6; margin-bottom: 16px;">{{ currentTechInfo.description }}</p>
                        
                        <div v-if="currentTechInfo.mutually_exclusive_with && currentTechInfo.mutually_exclusive_with.length > 0"
                             class="mutual-excl-note">
                            ⚠️ 互斥选择：与以下科技二选一，不可同时研究<br>
                            <strong v-for="eid in currentTechInfo.mutually_exclusive_with" :key="eid" style="color: #f87171;">
                                ❌ {{ gameState.tech_names?.[eid] || eid }}
                            </strong>
                        </div>
                        ...
                        <div style="background: var(--bg-card); padding: 14px; border-radius: 8px; margin-top: 14px;">
                            ...
                            <div v-for="(v, k) in currentTechInfo.effects" :key="k" style="color: var(--success); font-size: 0.9rem;">
                                ✦ {{ k }} +{{ v }}{{ typeof v === 'number' && v < 1 ? ' (' + (v * 100) + '%)' : '' }}
                            </div>
                        </div>

                        <div style="margin-top: 16px; display: flex; justify-content: space-between; align-items: center;">
                            <div style="font-size: 0.9rem;">研究消耗：<strong style="color: var(--warning);">📚 {{ currentTechInfo.cost }}</strong>
                                <span style="color: var(--text-dim); margin-left: 6px;">
                                    (当前: {{ gameState.tribe.knowledge }})
                                </span>
                            </div>
                        </div>
                    </div>
                    <div class="modal-actions">
                        ...
                        <button v-if="currentTechInfo.is_available" class="btn btn-primary" @click="researchTech(currentTechInfo.id)">
                            🧪 开始研究
                        </button>
                        <button v-else class="btn" style="background: #374151;" disabled>
                            {{ (gameState.researched_ids || []).includes(currentTechInfo.id) ? '✅ 已完成' : '🔒 条件未满足' }}
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div class="modal">
                    ...
                    <div class="modal-body">
                        <div style="background: var(--bg-card); padding: 10px 14px; border-radius: 8px; margin-bottom: 16px; font-size: 0.85rem;">
                            对方特产: <strong style="color: var(--success);">{{ RESOURCE_INFO[currentTradeTribe.specialty_resource]?.icon }} {{ RESOURCE_INFO[currentTradeTribe.specialty_resource]?.name }}</strong>
                        ...
                        <div class="trade-form-group">
                            ...
                            <div class="trade-form-row">
                                <select v-model="tradeForm.offered_resource" class="trade-select">
                                    <option v-for="(info, key) in RESOURCE_INFO" :key="key" :value="key">
                                        {{ info.icon }} {{ info.name }} ({{ gameState.tribe[key] }})
                                    </option>
                                </select>
                        ...
                        <div class="trade-preview">
                            ...
                            <div style="font-size: 1.2rem; font-weight: bold; color: var(--success);">
                                ~ {{ tradePreview }} {{ RESOURCE_INFO[tradeForm.requested_resource].icon }}
                            </div>
                        </div>
                    ...
                    <div class="modal-actions">
                        ...
                        <button class="btn btn-primary" @click="doTrade"
                                :disabled="tradeForm.offered_amount <= 0 || tradeForm.offered_amount > (gameState.tribe[tradeForm.offered_resource] || 0)">
                            🤝 完成交易
                        </button>
                    </div>
                </div>
