(function() {
    ...
    if (!Vue) {
        ...
        if (appEl) {
            appEl.innerHTML = '<div style="color:#ff4757;padding:40px;text-align:center;font-size:18px;">❌ Vue加载失败，请检查网络连接或刷新页面</div>';
        }
        return;
    }
    ...
    try {
        const app = createApp({
            ...
            errorHandler: function(err, vm, info) {
                ...
                alert('游戏渲染错误: ' + err.message + '\n请刷新页面重试');
            }
        });

        app.config.errorHandler = function(err, vm, info) {
            console.error('Vue Config Error:', err, info);
            const loadingEl = document.getElementById('vue-loading');
            if (loadingEl) {
                loadingEl.style.display = 'flex';
                loadingEl.innerHTML = '<div class="loading-content"><div class="loading-spinner">❌</div><p class="loading-text">游戏加载失败</p><p class="loading-hint">错误: ' + err.message + '</p><p class="loading-hint">请刷新页面重试，或按 F12 查看详细错误</p></div>';
            }
        };

        app.mount('#app');
        console.log('✅ 末日列车游戏已启动');
    } catch (e) {
        console.error('Vue init error:', e);
        const loadingEl = document.getElementById('vue-loading');
        if (loadingEl) {
            loadingEl.innerHTML = '<div class="loading-content"><div class="loading-spinner">❌</div><p class="loading-text">游戏初始化失败</p><p class="loading-hint">错误: ' + e.message + '</p><p class="loading-hint">请刷新页面重试</p></div>';
        }
    }
})();
