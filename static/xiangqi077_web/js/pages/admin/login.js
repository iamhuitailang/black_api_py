const AdminLoginPage = {
    ...
    <div class="auth-page">
      <div class="auth-card">
        ...
        <div class="form-group">
          <label>用户名</label>
          <input type="text" v-model="form.username" placeholder="请输入管理员用户名" @keyup.enter="handleLogin" />
        </div>
        <div class="form-group">
          <label>密码</label>
  ...
  setup() {
    ...
    async function handleLogin() {
      ...
      loading.value = true;
      try {
        const res = await XiangqiApi.adminLogin({ username: form.username, password: form.password });
        if (res.code === 0) {
          XiangqiAuth.setAdminToken(res.data.token);
          XiangqiAuth.setAdmin(res.data.admin);
