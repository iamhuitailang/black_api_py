...
<template>
  <div class="w-full">
    ...
    <div class="bar-bg h-4 rounded-sm overflow-hidden relative">
      <div
        class="hp-bar h-full transition-all duration-500 ease-out relative"
        :style="{ width: percentage + '%' }"
      >
        <div class="absolute inset-0 bg-gradient-to-b from-white/20 to-transparent"></div>
      </div>
      <div class="absolute inset-0 flex items-center justify-center">
        <span v-if="showText || !label" class="text-xs text-white font-mono drop-shadow-lg">{{ displayText }}</span>
      </div>
    </div>
  </div>
</template>
