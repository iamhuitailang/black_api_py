...
async def list_subordinates(supervisor_id: int):
...
async def publish_cycle(cycle_id: int):
...
async def submit_self_review(record_id: int, body: SelfReviewSubmit):
    kpi_business.submit_self_review(record_id, body.model_dump())
    return {"code": 0, "message": "success", "data": None}


@router.post("/records/{record_id}/supervisor-review")
async def submit_supervisor_review(record_id: int, body: SupervisorReviewSubmit):
...
async def get_employee_trend(employee_id: int):
