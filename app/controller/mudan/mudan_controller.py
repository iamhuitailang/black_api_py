...
class BannerAddRequest(BaseModel):
    image_url: str = Field(..., description="图片链接")
    jump_url: str = Field(default='', description="跳转链接")
...
class BannerUpdateRequest(BaseModel):
    ...
    jump_url: Optional[str] = Field(default=None, description="跳转链接")
...
class MudanController:
    ...
    def ActionMudanBannerItemGet(self, request: Request, id: int = Query(..., ge=1, description="Banner ID")):
        """
        获取单个banner接口
        GET /api/mudan/banner/item/get
        参数: id - Banner ID
        """
        return self.banner_business.get_banner_by_id(id)

    def ActionMudanBannerItemAddPost(self, request: Request, body: BannerAddRequest):
        """
        添加单个banner接口
        POST /api/mudan/banner/item/add
        动态添加单个banner，支持图片链接和跳转链接
        """
        return self.banner_business.add_single_banner(
            image_url=body.image_url,
            jump_url=body.jump_url
        )

    def ActionMudanBannerItemUpdatePost(self, request: Request, body: BannerUpdateRequest):
        """
        更新单个banner接口
        POST /api/mudan/banner/item/update
        更新单个banner信息
        """
        return self.banner_business.update_single_banner(
            record_id=body.id,
            image_url=body.image_url,
            jump_url=body.jump_url
        )
    ...
    def ActionMudanCommercialProductAddPost(self, request: Request, body: ProductAddRequest):
        """
    ...
    def ActionMudanCommercialProductUpdatePost(self, request: Request, body: ProductUpdateRequest):
        """
