...
from app.model.jianshen import JianshenAdminModel, JianshenAdminTokenModel


class JianshenAdminBusiness:
    def __init__(self):
        self.admin_model = JianshenAdminModel()
        self.token_model = JianshenAdminTokenModel()
    ...
    def get_current_admin(self, token: str) -> Dict[str, Any]:
        ...
        if admin:
            return {'code': 0, 'msg': 'success', 'data': admin}
    ...
    def create_admin(self, username: str, password: str, real_name: str = '') -> Dict[str, Any]:
        ...
        if admin_id > 0:
            ...
            return {'code': 0, 'msg': '创建成功', 'data': self.admin_model.to_dict(admin)}
        return {'code': 1, 'msg': '创建失败', 'data': None}

    def change_password(self, admin_id: int, old_password: str, new_password: str) -> Dict[str, Any]:
        admin = self.admin_model.get_by_id(admin_id)
        if not admin:
            return {'code': 1, 'msg': '管理员不存在', 'data': None}
        verify = self.admin_model.verify_password(admin.get('username', ''), old_password)
        if verify is None:
    ...
    def update_profile(self, admin_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not admin:
            return {'code': 1, 'msg': '管理员不存在', 'data': None}
        self.admin_model.update_profile(admin_id, data)
        updated = self.admin_model.get_by_id(admin_id)
        return {'code': 0, 'msg': '更新成功', 'data': self.admin_model.to_dict(updated)}
    ...
    def delete_admin(self, admin_id: int) -> Dict[str, Any]:
        ...
        if admin.get('username') == 'admin':
            return {'code': 1, 'msg': '不能删除超级管理员', 'data': None}
        self.token_model.delete_by_admin_id(admin_id)
        if self.admin_model.delete(admin_id) > 0:
    ...
    def disable_admin(self, admin_id: int) -> Dict[str, Any]:
        ...
        if not admin:
            return {'code': 1, 'msg': '管理员不存在', 'data': None}
        if admin.get('username') == 'admin':
            return {'code': 1, 'msg': '不能禁用超级管理员', 'data': None}
        self.admin_model.update_status(admin_id, JianshenAdminModel.STATUS_DISABLED)
        self.token_model.delete_by_admin_id(admin_id)
    ...
    def enable_admin(self, admin_id: int) -> Dict[str, Any]:
        ...
        if not admin:
            return {'code': 1, 'msg': '管理员不存在', 'data': None}
        self.admin_model.update_status(admin_id, JianshenAdminModel.STATUS_ACTIVE)
