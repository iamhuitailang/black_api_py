...
from app.model.shipu_077_model import CategoryModel


class ShipuCategoryBusiness:
    def __init__(self):
        self.category_model = CategoryModel()

    def create(self, name: str, description: str = '', icon: str = '', sort_order: int = 0) -> Dict[str, Any]:
        if not name:
            return {
                'code': 1,
                'msg': '分类名称不能为空',
        ...
        category_id = self.category_model.create(name, description, icon, sort_order)
        if category_id > 0:
    ...
    def update(self, category_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'name' in data and data['name'] != category.get('name'):
