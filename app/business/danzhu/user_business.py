...
from app.model.danzhu_model import UserModel, UserTokenModel
import re


class DanzhuUserBusiness:
    def __init__(self):
    ...
    def get_leaderboard(self, page: int = 1, page_size: int = 10) -> Dict[str, Any]:
        ...
        items = []
        for idx, user in enumerate(result.get('items', [])):
            user_dict = self.user_model.to_public_dict(user)
            user_dict['rank'] = (page - 1) * page_size + idx + 1
    ...
    def get_user_rank(self, user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'data': {
                'rank': rank,
                'highest_score': user.get('highest_score', 0)
            }
        }
