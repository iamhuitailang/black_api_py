...
from app.model.gq_model import GqUserModel, GqTokenModel
from app.model.gq_model import GqTrackModel, GqInstrumentModel, GqMagicModel
from app.model.gq_model import GqUserInstrumentModel, GqUserMagicModel, GqUserProgressModel
import re


class GqUserBusiness:
    def __init__(self):
        self.user_model = GqUserModel()
        self.token_model = GqTokenModel()
    ...
    def register(self, username: str, password: str, nickname: str = '') -> Dict[str, Any]:
        ...
        if user_id > 0:
            track_model = GqTrackModel()
            progress_model = GqUserProgressModel()
            default_tracks = track_model.get_default_tracks()
            for track in default_tracks:
                progress_model.create(user_id, track['id'], is_unlocked=1)

            instrument_model = GqInstrumentModel()
            user_instrument_model = GqUserInstrumentModel()
            default_instruments = instrument_model.get_default_instruments()
            for instrument in default_instruments:
                user_instrument_model.create(user_id, instrument['id'], is_equipped=1)

            magic_model = GqMagicModel()
            user_magic_model = GqUserMagicModel()
            default_magics = magic_model.get_default_magics()
            for magic in default_magics:
                user_magic_model.create(user_id, magic['id'], is_equipped=1)
    ...
    def add_currency(self, user_id: int, coins: int = 0, gems: int = 0) -> Dict[str, Any]:
        user = self.user_model.get_by_id(user_id)
        if not user:
            return {
                'code': 1,
        ...
        affected = self.user_model.update_currency(user_id, coins, gems)
        if affected > 0:
            updated_user = self.user_model.get_by_id(user_id)
            return {
                'code': 0,
    ...
    def add_experience(self, user_id: int, exp: int) -> Dict[str, Any]:
        ...
        affected = self.user_model.add_exp(user_id, exp)
        if affected > 0:
            updated_user = self.user_model.get_by_id(user_id)
            ...
            return {
                'code': 0,
                'msg': '升级成功' if leveled_up else '经验增加成功',
                'data': {
