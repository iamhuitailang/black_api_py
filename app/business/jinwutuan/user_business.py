...
from app.model.jinwutuan import UserModel, TokenModel
import re


class JinwutuanUserBusiness:
    def __init__(self):
    ...
    def update_user_status(self, user_id: int, status: int) -> Dict[str, Any]:
        ...
        if status not in [self.user_model.STATUS_ACTIVE, self.user_model.STATUS_MUTED, self.user_model.STATUS_BANNED]:
            return {
                'code': 1,
                'msg': '无效的状态值',
                'data': None
