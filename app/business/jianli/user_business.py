...
from app.model.jianli import UserModel, TokenModel
import re
...
class UserBusiness:
    ...
    def register(self, username: str, password: str, phone: str = '',
                 email: str = '', nickname: str = '') -> Dict[str, Any]:
        ...
        user_id = self.user_model.create(username, password, phone, email, nickname)
        if user_id > 0:
    ...
    def update_profile(self, user_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'phone' in data and not self._validate_phone(data.get('phone', '')):
            return {
                'code': 1,
                'msg': '手机号格式不正确',
                'data': None
        ...
        if 'email' in data and not self._validate_email(data.get('email', '')):
            return {
                'code': 1,
                'msg': '邮箱格式不正确',
                'data': None
