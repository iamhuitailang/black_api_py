...
from app.model.jianli import AdminModel, AdminTokenModel
import re
...
class AdminBusiness:
    ...
    def get_admin_list(self, page: int = 1, page_size: int = 10, status: int = None) -> Dict[str, Any]:
        result = self.admin_model.get_all(page, page_size, status)
    ...
    def update_admin_status(self, admin_id: int, status: int) -> Dict[str, Any]:
        ...
        if affected > 0:
            if status == self.admin_model.STATUS_DISABLED:
                self.token_model.delete_by_admin_id(admin_id)
    ...
    def delete_admin(self, admin_id: int) -> Dict[str, Any]:
        ...
        if admin.get('role') == self.admin_model.ROLE_SUPER:
            return {
                'code': 1,
                'msg': '超级管理员不可删除',
                'data': None
