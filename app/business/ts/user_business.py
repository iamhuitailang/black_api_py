...
from app.model.ts import TsUserModel, TsTokenModel, TsRecordModel, TsUserAchievementModel
import re


class TsUserBusiness:
    def __init__(self):
        self.user_model = TsUserModel()
        self.token_model = TsTokenModel()
    ...
    def login(self, phone: str, password: str) -> Dict[str, Any]:
        ...
        if user.get('status') == self.user_model.STATUS_INACTIVE:
            return {
                'code': 1,
                'msg': '账号已被禁用，请联系管理员',
                'data': None
    ...
    def update_daily_goal(self, user_id: int, daily_goal: int) -> Dict[str, Any]:
        if daily_goal <= 0:
            return {
                'code': 1,
                'msg': '每日目标必须大于0',
        ...
        affected = self.user_model.update_profile(user_id, {'daily_goal': daily_goal})
        if affected >= 0:
            updated_user = self.user_model.get_by_id(user_id)
            return {
                'code': 0,
    ...
    def get_ranking(self, limit: int = 10, order_by: str = 'total_count') -> Dict[str, Any]:
        rankings = self.user_model.get_ranking(limit, order_by)
        return {
            'code': 0,
            'msg': 'success',
            'data': rankings
    ...
    def get_user_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        achievement_count = self.user_achievement_model.get_achievement_count(user_id)

        from app.model.ts.achievement import TsAchievementModel
        achievement_model = TsAchievementModel()
        total_achievements = len(achievement_model.get_all())
