...
from app.model.zhaobutong_model import ZbtUserModel, ZbtTokenModel
import re


class ZbtUserBusiness:
    def __init__(self):
        self.user_model = ZbtUserModel()
        self.token_model = ZbtTokenModel()
    ...
    def register(self, username: str, password: str, nickname: str = '') -> Dict[str, Any]:
        if not self._validate_username(username):
            return {'code': 1, 'msg': '用户名需3位以上，仅含字母数字下划线', 'data': None}
    ...
    def update_profile(self, user_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'code': 0,
            'msg': '更新成功',
            'data': self.user_model.to_public_dict(updated)
        }
    ...
    def update_user_status(self, user_id: int, status: int) -> Dict[str, Any]:
        ...
        if affected > 0:
            updated = self.user_model.get_by_id(user_id)
            return {'code': 0, 'msg': '状态更新成功', 'data': self.user_model.to_public_dict(updated)}
