...
from app.model.chouchou_model import UserModel, TokenModel, ThemeModel, SettingModel, HighScoreModel
import re
...
class UserBusiness:
    ...
    def register(self, username: str, password: str, nickname: str = '', phone: str = '') -> Dict[str, Any]:
        ...
        user_id = self.user_model.create(username, password, nickname, phone or None)
        if user_id > 0:
            ThemeModel.init_default_themes(user_id)
            self.setting_model.create_default(user_id)
    ...
    def get_rankings(self, limit: int = 10) -> Dict[str, Any]:
        users = self.user_model.get_rankings(limit)
        items = [self.user_model.to_public_dict(user) for user in users]
        return {
            'code': 0,
            'msg': 'success',
            'data': items
    ...
    def delete_user(self, user_id: int) -> Dict[str, Any]:
        ...
        self.token_model.delete_by_user_id(user_id)
        self.theme_model.delete_by_user(user_id)
        self.setting_model.delete_by_user(user_id)
        self.high_score_model.delete_by_user(user_id)
