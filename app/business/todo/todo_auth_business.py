...
from app.model.todo import TodoUserModel, TodoTokenModel
import re


class TodoAuthBusiness:
    def __init__(self):
        self.user_model = TodoUserModel()
        self.token_model = TodoTokenModel()
    ...
    def login(self, login_name: str, password: str) -> Dict[str, Any]:
        ...
        user = self.user_model.verify_password(login_name, password)
        if user is None:
            return {
                'code': 1,
                'msg': '用户名或密码错误',
