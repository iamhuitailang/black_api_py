...
from app.model.doudizhu_model import AdminModel, DoudizhuAdminTokenModel
import re


class DoudizhuAdminBusiness:
    def __init__(self):
        self.admin_model = AdminModel()
        self.token_model = DoudizhuAdminTokenModel()
    ...
    def create_admin(self, username: str, password: str, real_name: str = '', role: int = 1) -> Dict[str, Any]:
        if not self._validate_username(username):
            return {
                'code': 1,
                'msg': '用户名格式不正确',
    ...
    def delete_admin(self, admin_id: int) -> Dict[str, Any]:
        ...
        if admin.get('role') == AdminModel.ROLE_SUPER:
            return {
                'code': 1,
                'msg': '不能删除超级管理员',
                'data': None
