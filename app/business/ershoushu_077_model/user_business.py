...
from app.model.ershoushu_077_model import ErshoushuUserModel, ErshoushuTokenModel, ErshoushuNotificationModel
import re


class ErshoushuUserBusiness:
    def __init__(self):
        self.user_model = ErshoushuUserModel()
        self.token_model = ErshoushuTokenModel()
    ...
    def register(self, username: str, password: str, nickname: str = '',
        ...
        if not self._validate_username(username):
            return {'code': 1, 'msg': '用户名至少3位，只能包含字母数字下划线', 'data': None}
        ...
        if phone:
            ...
            if existing_phone:
                return {'code': 1, 'msg': '该手机号已注册', 'data': None}

        user_id = self.user_model.create(username, password, nickname, phone, ErshoushuUserModel.ROLE_USER)
        if user_id > 0:
            ...
            self.notification_model.create(
                user_id=user_id,
                title='欢迎加入二手书交易',
                content='欢迎注册二手书交易系统！您可以浏览和发布二手书籍，开始您的阅读之旅。',
                ntype=ErshoushuNotificationModel.TYPE_SYSTEM,
                related_id=0
            )
