...
from app.model.jiaoyi import UserModel, TokenModel
import re


class JiaoyiUserBusiness:
    def __init__(self):
    ...
    def register(self, username: str, phone: str, password: str, nickname: str = '',
                 school: str = '', major: str = '', grade: str = '', role: str = 'buyer') -> Dict[str, Any]:
        if not self._validate_username(username):
            return {
                'code': 1,
                'msg': '用户名格式不正确，长度3-20位，只能包含字母数字下划线',
                'data': None
            }
        ...
        user_id = self.user_model.create(username, phone, password, nickname, school, major, grade, role)
        if user_id > 0:
    ...
    def get_user_list(self, page: int = 1, page_size: int = 10,
                      status: int = None, school: str = None, keyword: str = None) -> Dict[str, Any]:
        result = self.user_model.get_all(page, page_size, status, school, keyword)
