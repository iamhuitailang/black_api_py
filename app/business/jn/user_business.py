...
from app.model.jn import UserModel, JnTokenModel
import re


class JnUserBusiness:
    def __init__(self):
        self.user_model = UserModel()
        self.token_model = JnTokenModel()
    ...
    def reset_credit(self, user_id: int) -> Dict[str, Any]:
        user = self.user_model.get_by_id(user_id)
        if not user:
            return {
                'code': 1,
        ...
        affected = self.user_model.reset_credit(user_id)
        if affected > 0:
            updated_user = self.user_model.get_by_id(user_id)
            return {
                'code': 0,
