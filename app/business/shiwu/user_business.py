...
from app.model.shiwu_model import UserModel, TokenModel
import re
...
class UserBusiness:
    ...
    def register(self, phone: str, password: str, nickname: str = '', real_name: str = '',
                 student_id: str = '', college: str = '', contact: str = '') -> Dict[str, Any]:
        ...
        if student_id:
            existing_student = self.user_model.get_by_student_id(student_id)
            if existing_student:
                return {
                    'code': 1,
                    'msg': '该学号已注册',
        ...
        user_id = self.user_model.create(phone, password, nickname, real_name, 
                                        student_id, college, contact)
        if user_id > 0:
    ...
    def get_user_list(self, page: int = 1, page_size: int = 10,
                      status: int = None, college: str = None, keyword: str = None) -> Dict[str, Any]:
        result = self.user_model.get_all(page, page_size, status, college, keyword)
