...
from app.model.shiwu_model import AdminModel, AdminTokenModel
import re
...
class AdminBusiness:
    ...
    def create_admin(self, current_admin_id: int, username: str, password: str, 
                     real_name: str = '', role: str = 'normal') -> Dict[str, Any]:
        current_admin = self.admin_model.get_by_id(current_admin_id)
        if not current_admin or current_admin.get('role') != AdminModel.ROLE_SUPER:
            return {
                'code': 1,
                'msg': '无权限操作',
                'data': None
        ...
        admin_id = self.admin_model.create(username, password, real_name, role)
        if admin_id > 0:
    ...
    def get_admin_list(self, page: int = 1, page_size: int = 10,
        ...
        result = self.admin_model.get_all(page, page_size, status, role)
    ...
    def update_admin_status(self, current_admin_id: int, admin_id: int, status: int) -> Dict[str, Any]:
        current_admin = self.admin_model.get_by_id(current_admin_id)
        if not current_admin or current_admin.get('role') != AdminModel.ROLE_SUPER:
            return {
                'code': 1,
        ...
        if affected > 0:
            if status == AdminModel.STATUS_DISABLED:
                self.token_model.delete_by_admin_id(admin_id)
            updated_admin = self.admin_model.get_by_id(admin_id)
            return {
                'code': 0,
    ...
    def delete_admin(self, current_admin_id: int, admin_id: int) -> Dict[str, Any]:
        current_admin = self.admin_model.get_by_id(current_admin_id)
        if not current_admin or current_admin.get('role') != AdminModel.ROLE_SUPER:
            return {
                'code': 1,
