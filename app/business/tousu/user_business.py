...
from app.model.tousu_model import UserModel, TokenModel, LogModel
import re


class TousuUserBusiness:
    def __init__(self):
    ...
    def register(self, username: str, phone: str, password: str, role: str = 'student',
                 nickname: str = '', department_id: int = 0) -> Dict[str, Any]:
        if not self._validate_username(username):
            return {
                'code': 1,
                'msg': '用户名格式不正确（3-20位，仅含字母、数字、下划线）',
                'data': None
            }
        ...
        if role not in [UserModel.ROLE_STUDENT, UserModel.ROLE_STAFF]:
