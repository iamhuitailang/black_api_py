...
from app.model.xiangqi077_model import XiangqiUserModel, XiangqiTokenModel
import re


class XiangqiUserBusiness:
    def __init__(self):
        self.user_model = XiangqiUserModel()
        self.token_model = XiangqiTokenModel()
    ...
    def register(self, username: str, password: str, nickname: str = '') -> Dict[str, Any]:
        if not self._validate_username(username):
            return {'code': 1, 'msg': '用户名至少3个字符，只允许字母数字下划线中文', 'data': None}
        ...
        if user_id > 0:
            token = self.token_model.create_token(user_id, hours=72)
            user = self.user_model.get_by_id(user_id)
            return {
                'code': 0,
                'msg': '注册成功',
                'data': {'user': self.user_model.to_public_dict(user), 'token': token}
            }
    ...
    def login(self, username: str, password: str) -> Dict[str, Any]:
        ...
        if user.get('status') == self.user_model.STATUS_BANNED:
            return {'code': 1, 'msg': '账号已被封号，请联系管理员', 'data': None}
        self.token_model.delete_by_user_id(user.get('id'))
        token = self.token_model.create_token(user.get('id'), hours=72)
        user_full = self.user_model.get_by_id(user.get('id'))
        return {
            'code': 0,
            'msg': '登录成功',
            'data': {'user': self.user_model.to_public_dict(user_full), 'token': token}
        }
    ...
    def change_password(self, user_id: int, old_password: str, new_password: str) -> Dict[str, Any]:
        ...
        if not user:
            return {'code': 1, 'msg': '用户不存在', 'data': None}
        verify_result = self.user_model.verify_password(user.get('username', ''), old_password)
    ...
    def update_user_status(self, user_id: int, status: int) -> Dict[str, Any]:
        ...
        if affected > 0:
            updated_user = self.user_model.get_by_id(user_id)
            return {'code': 0, 'msg': '状态更新成功', 'data': self.user_model.to_public_dict(updated_user)}
        return {'code': 1, 'msg': '状态更新失败', 'data': None}

    def mute_user(self, user_id: int) -> Dict[str, Any]:
        return self.update_user_status(user_id, self.user_model.STATUS_MUTED)
