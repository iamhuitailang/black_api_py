...
from app.model.ty_model import UserModel, TokenModel
import re


class TyAuthBusiness:
    def __init__(self):
    ...
    def register(self, phone: str, password: str, nickname: str = '') -> Dict[str, Any]:
        ...
        if user_id > 0:
            ...
            self._init_default_data(user_id)
    ...
    def _init_default_data(self, user_id: int):
        from app.model.ty_model import UserSkillModel, SkillModel
        user_skill_model = UserSkillModel()
        skill_model = SkillModel()

        skills = skill_model.get_available_for_level(1, page_size=100)
        for skill in skills.get('items', []):
            if skill.get('unlock_level', 1) == 1:
                user_skill_model.unlock_skill(user_id, skill.get('id'))
