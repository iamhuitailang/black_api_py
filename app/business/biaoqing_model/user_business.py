...
from app.model.biaoqing_model import UserModel, TokenModel, PointLogModel
import re
...
class BqUserBusiness:
    ...
    def register(self, username: str, email: str, password: str, nickname: str = '', phone: str = '') -> Dict[str, Any]:
        ...
        existing_email = self.user_model.get_by_email(email)
        if existing_email:
            return {
                'code': 1,
                'msg': '该邮箱已注册',
        ...
        user_id = self.user_model.create(username, email, password, nickname, phone)
        if user_id > 0:
            self.point_log_model.create(user_id, 100, PointLogModel.TYPE_REGISTER, '注册奖励')
    ...
    def sign_in(self, user_id: int) -> Dict[str, Any]:
        from datetime import date
        today = date.today().isoformat()

        point_logs = self.point_log_model.get_by_user_id(user_id, page=1, page_size=1)
        has_signed_today = False
        for log in point_logs.get('items', []):
            if log.get('type') == PointLogModel.TYPE_SIGN_IN and log.get('created_at', '').startswith(today):
        ...
        self.point_log_model.create(user_id, 10, PointLogModel.TYPE_SIGN_IN, '每日签到')
        user = self.user_model.get_by_id(user_id)
    ...
    def get_point_logs(self, user_id: int, page: int = 1, page_size: int = 20, type: int = None) -> Dict[str, Any]:
        result = self.point_log_model.get_by_user_id(user_id, page, page_size, type)
        items = [self.point_log_model.to_dict(item) for item in result.get('items', [])]
