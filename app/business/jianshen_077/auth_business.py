...
from app.model.jianshen_077_model import JianshenUserModel, JianshenTokenModel
import re


class JianshenAuthBusiness:
    def __init__(self):
    ...
    def register(self, username: str, password: str, nickname: str = '',
                 phone: str = '', role: int = 0) -> Dict[str, Any]:
        ...
        user_id = self.user_model.create(username, password, nickname, phone, role)
        if user_id > 0:
    ...
    def update_profile(self, user_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data.get('phone') and not self._validate_phone(data.get('phone')):
            return {
                'code': 1,
                'msg': '手机号格式不正确',
                'data': None
    ...
    def get_user_list(self, page: int = 1, page_size: int = 10,
                      role: int = None, status: int = None, keyword: str = None) -> Dict[str, Any]:
