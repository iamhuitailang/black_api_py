...
from app.model.wangzhe_model import UserModel, TokenModel, UserHeroModel, AchievementModel, UserAchievementModel
import re


class WangzheUserBusiness:
    def __init__(self):
    ...
    def register(self, username: str, password: str, nickname: str = '') -> Dict[str, Any]:
        ...
        if user_id > 0:
            self.user_hero_model.create(user_id, 4)

            token = self.token_model.create_token(user_id, hours=24)
            user = self.user_model.get_by_id(user_id)
            rank = self.user_model.get_user_rank(user_id)
            user_dict = self.user_model.to_public_dict(user)
            user_dict['rank'] = rank

            self._check_level_achievement(user_id)
    ...
    def login(self, username: str, password: str) -> Dict[str, Any]:
        ...
        rank = self.user_model.get_user_rank(user.get('id'))
        user_dict = self.user_model.to_public_dict(user_full)
        user_dict['rank'] = rank
    ...
    def update_profile(self, user_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if affected >= 0:
            ...
            rank = self.user_model.get_user_rank(user_id)
            user_dict = self.user_model.to_public_dict(updated_user)
            user_dict['rank'] = rank
            return {
                'code': 0,
                'msg': '更新成功',
    ...
    def get_user_list(self, page: int = 1, page_size: int = 10,
        ...
        for item in result.get('items', []):
            ...
            user_dict['rank'] = self.user_model.get_user_rank(item.get('id'))
    ...
    def _check_level_achievement(self, user_id: int):
        user = self.user_model.get_by_id(user_id)
        if not user:
            return

        level = user.get('level', 1)
        achievements = self.achievement_model.get_all(type='level').get('items', [])
        for achievement in achievements:
            if level >= achievement.get('target_value', 0):
                self.user_achievement_model.update_progress(
                    user_id, achievement.get('id'), level, achievement.get('target_value', 0)
                )

    def _check_rank_achievement(self, user_id: int):
        user = self.user_model.get_by_id(user_id)
        if not user:
            return

        points = user.get('ranking_points', 1000)
        achievements = self.achievement_model.get_all(type='rank').get('items', [])
        for achievement in achievements:
            if points >= achievement.get('target_value', 0):
                self.user_achievement_model.update_progress(
                    user_id, achievement.get('id'), points, achievement.get('target_value', 0)
                )
