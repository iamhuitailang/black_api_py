...
from app.model.zuma_model import ZumaUserModel, ZumaTokenModel, ZumaUserItemModel
import re


class ZumaUserBusiness:
    def __init__(self):
        self.user_model = ZumaUserModel()
        self.token_model = ZumaTokenModel()
        self.item_model = ZumaUserItemModel()
    ...
    def register(self, username: str, password: str, nickname: str = '') -> Dict[str, Any]:
        ...
        if user_id > 0:
            self.item_model.add_item(user_id, 'slow_time', 3)
            self.item_model.add_item(user_id, 'aim', 5)
    ...
    def get_rankings(self, limit: int = 100) -> Dict[str, Any]:
        rankings = self.user_model.get_rankings(limit)
        return {
            'code': 0,
            'msg': 'success',
            'data': rankings
