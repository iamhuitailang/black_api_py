...
from app.model.saiche_model import UserModel, TokenModel, UserCarModel, CarModel
import re


class SaicheUserBusiness:
    def __init__(self):
    ...
    def register(self, phone: str, password: str, nickname: str = '') -> Dict[str, Any]:
        ...
        if user_id > 0:
            default_car = self.car_model.get_default_car()
            if default_car:
                self.user_car_model.create(user_id, default_car['id'], is_active=1)
    ...
    def update_user_status(self, user_id: int, status: int) -> Dict[str, Any]:
        ...
        if affected > 0:
            if status == self.user_model.STATUS_BANNED:
                self.token_model.delete_by_user_id(user_id)
            updated_user = self.user_model.get_by_id(user_id)
            return {
                'code': 0,
