...
from app.model.poan_model import UserModel, TokenModel
import re


class PoanUserBusiness:
    def __init__(self):
    ...
    def add_experience(self, user_id: int, exp: int) -> Dict[str, Any]:
        ...
        affected = self.user_model.add_experience(user_id, exp)
        if affected > 0:
            level_updated = False
            if new_level > current_level:
                level_affected = self.user_model.update_level(user_id, new_level)
                level_updated = level_affected > 0
