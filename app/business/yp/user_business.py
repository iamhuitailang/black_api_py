...
from app.model.yp_model import (
    UserModel, YpTokenModel, UserCharacterModel,
    UserMusicModel, GameStateModel
)
import re


class YpUserBusiness:
    def __init__(self):
        self.user_model = UserModel()
        self.token_model = YpTokenModel()
    ...
    def register(self, username: str, email: str, password: str, nickname: str = '') -> Dict[str, Any]:
        ...
        user_id = self.user_model.create(username, email, password, nickname)
        if user_id > 0:
            self.user_character_model.create(user_id, 1)
            self.user_character_model.set_using_character(user_id, 1)
