...
class JianshenAdminTokenModel:
    TABLE_NAME = 'tb_jianshen_admin_token'
    ...
    def create_table(cls):
        ...
        """
        db.execute(sql)
        db.execute(f"CREATE INDEX IF NOT EXISTS idx_{cls.TABLE_NAME}_token ON {cls.TABLE_NAME}(token)")
        db.execute(f"CREATE INDEX IF NOT EXISTS idx_{cls.TABLE_NAME}_admin_id ON {cls.TABLE_NAME}(admin_id)")
    ...
    def get_admin_by_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        if not token_record:
            return None
        from app.model.jianshen.admin import JianshenAdminModel
        admin_model = JianshenAdminModel()
    ...
    def delete_by_admin_id(self, admin_id: int) -> int:
        return self.exec.execute_raw(f"DELETE FROM {self.TABLE_NAME} WHERE admin_id = ?", (admin_id,))
