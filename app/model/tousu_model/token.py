...
class TokenModel:
    TABLE_NAME = 'tb_tousu_model_token'
    ...
    def get_user_by_token(self, token: str) -> Optional[Dict[str, Any]]:
        sql = f"""
            SELECT t.*, u.username, u.phone, u.nickname, u.role, u.department_id, u.avatar, u.status
            FROM {self.TABLE_NAME} t
            JOIN tb_tousu_model_users u ON t.user_id = u.id
            WHERE t.token = ? AND t.expires_at > ?
        """
        result = self.db.fetch_one(sql, (token, datetime.now().isoformat()))
        if result:
            return {
