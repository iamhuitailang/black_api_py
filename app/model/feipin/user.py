...
class UserModel:
    ...
    def create(self, phone: str, password: str, role: str = ROLE_USER,
               nickname: str = '', avatar: str = '') -> int:
        ...
        data = {
            ...
            'avatar': avatar,
            'balance': 0.0,
            'status': self.STATUS_ACTIVE if role == self.ROLE_USER else self.STATUS_PENDING,
            'created_at': now,
            'updated_at': now
        }
    ...
    def update_profile(self, user_id: int, data: Dict[str, Any]) -> int:
        ...
        update_data = {k: v for k, v in data.items() if k in [
            'nickname', 'avatar', 'id_card', 'id_card_photo'
        ]}
    ...
    def verify_collector(self, user_id: int, approved: bool, note: str = '') -> int:
        user = self.get_by_id(user_id)
        if not user or user.get('role') != self.ROLE_COLLECTOR:
            return 0
        now = datetime.now().isoformat()
        status = self.STATUS_ACTIVE if approved else self.STATUS_BANNED
        data = {
            'status': status,
            'verify_note': note,
            'updated_at': now
    ...
    def get_collectors(self, page: int = 1, page_size: int = 10) -> Dict[str, Any]:
        return self.get_all(page, page_size, role=self.ROLE_COLLECTOR, status=self.STATUS_ACTIVE)
    ...
    def to_admin_dict(self, user: Dict[str, Any]) -> Dict[str, Any]:
        return {
            'id': user.get('id'),
            'phone': user.get('phone'),
            'role': user.get('role'),
            ...
            'id_card_photo': user.get('id_card_photo'),
            'verify_note': user.get('verify_note'),
            'created_at': user.get('created_at'),
