...
class UserModel:
    ...
    def create_table(cls):
        ...
        sql = f"""
            CREATE TABLE IF NOT EXISTS {cls.TABLE_NAME} (
                ...
                paint_count INTEGER DEFAULT 10,
                canvas_count INTEGER DEFAULT 5,
    ...
    def create(self, phone: str, password: str, nickname: str = '') -> int:
        ...
        data = {
            ...
            'salt': salt,
            'nickname': nickname or f'涂鸦战士{phone[-4:]}',
            'avatar': '',
            'level': 1,
            'exp': 0,
            'gold': 100,
    ...
    def verify_password(self, phone: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        if password_hash == user.get('password_hash'):
            return {
                ...
                'exp': user.get('exp'),
                'gold': user.get('gold'),
                'diamond': user.get('diamond'),
                'paint_count': user.get('paint_count'),
                'canvas_count': user.get('canvas_count'),
    ...
    def add_exp(self, user_id: int, exp: int) -> Dict[str, Any]:
        ...
        affected = self.exec.update_by_id(user_id, data)
        return {
            'success': affected > 0,
            'level_up': level_up,
            'new_level': new_level,
    ...
    def add_gold(self, user_id: int, amount: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
    ...
    def add_diamond(self, user_id: int, amount: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0

        new_diamond = max(0, user.get('diamond', 0) + amount)
        now = datetime.now().isoformat()
        data = {
            'diamond': new_diamond,
            'updated_at': now
    ...
    def add_paint(self, user_id: int, amount: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0

        new_paint = max(0, user.get('paint_count', 0) + amount)
        now = datetime.now().isoformat()
        data = {
            'paint_count': new_paint,
            'updated_at': now
        }
        return self.exec.update_by_id(user_id, data)

    def add_canvas(self, user_id: int, amount: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0

        new_canvas = max(0, user.get('canvas_count', 0) + amount)
        now = datetime.now().isoformat()
        data = {
            'canvas_count': new_canvas,
            'updated_at': now
        }
        return self.exec.update_by_id(user_id, data)

    def update_battle_result(self, user_id: int, win: bool) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
        ...
        if win:
            data = {
                'win_count': user.get('win_count', 0) + 1,
                'updated_at': now
            }
        else:
            data = {
                'lose_count': user.get('lose_count', 0) + 1,
                'updated_at': now
            }
        return self.exec.update_by_id(user_id, data)

    def use_paint_and_canvas(self, user_id: int, paint_used: int = 1, canvas_used: int = 1) -> bool:
        user = self.get_by_id(user_id)
        if not user:
            return False

        if user.get('paint_count', 0) < paint_used or user.get('canvas_count', 0) < canvas_used:
            return False

        now = datetime.now().isoformat()
        data = {
            'paint_count': user.get('paint_count', 0) - paint_used,
            'canvas_count': user.get('canvas_count', 0) - canvas_used,
            'updated_at': now
        }
        affected = self.exec.update_by_id(user_id, data)
        return affected > 0
