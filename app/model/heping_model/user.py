...
class HepingUserModel:
    TABLE_NAME = 'tb_heping_model_users'
    ...
    def create(self, username: str, password: str, nickname: str = '', avatar: str = '') -> int:
        ...
        data = {
            ...
            'salt': salt,
            'nickname': nickname or f'玩家{secrets.token_hex(2)}',
            'avatar': avatar,
            'level': 1,
            'exp': 0,
            'kills': 0,
    ...
    def update_stats(self, user_id: int, kills: int = 0, deaths: int = 0,
                     wins: int = 0, games_played: int = 0) -> int:
        ...
        now = datetime.now().isoformat()
        data = {
            'kills': user.get('kills', 0) + kills,
            'deaths': user.get('deaths', 0) + deaths,
            'wins': user.get('wins', 0) + wins,
            'games_played': user.get('games_played', 0) + games_played,
            'updated_at': now
        }
        return self.exec.update_by_id(user_id, data)
    ...
    def to_public_dict(self, user: Dict[str, Any]) -> Dict[str, Any]:
        ...
        win_rate = round(user.get('wins', 0) / user.get('games_played', 1) * 100, 1) if user.get('games_played', 0) > 0 else 0.0
        ...
        return {
            ...
            'win_rate': win_rate,
            'status': user.get('status'),
            'status_text': status_map.get(user.get('status'), '未知'),
