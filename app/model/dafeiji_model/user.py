...
class DafeijiUserModel:
    TABLE_NAME = 'tb_dafeiji_model_user'
    ...
    def create(self, username: str, password: str, nickname: str = '', role: str = 'user') -> int:
    ...
    def increment_games_played(self, user_id: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
        new_count = user.get('games_played', 0) + 1
        now = datetime.now().isoformat()
        data = {
            'games_played': new_count,
            'updated_at': now
        }
        return self.exec.update_by_id(user_id, data)
