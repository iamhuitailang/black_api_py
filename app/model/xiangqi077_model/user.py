...
class XiangqiUserModel:
    TABLE_NAME = 'tb_xiangqi077_model_user'
    ...
    def create(self, username: str, password: str, nickname: str = '') -> int:
        ...
        data = {
            ...
            'salt': salt,
            'nickname': nickname or f'棋手{secrets.token_hex(3)}',
            'avatar': '',
            'score': 1000,
            'wins': 0,
            'losses': 0,
    ...
    def update_score(self, user_id: int, delta: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
        new_score = max(0, user.get('score', 0) + delta)
        now = datetime.now().isoformat()
        data = {'score': new_score, 'updated_at': now}
        return self.exec.update_by_id(user_id, data)

    def increment_wins(self, user_id: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
        now = datetime.now().isoformat()
        data = {'wins': user.get('wins', 0) + 1, 'updated_at': now}
        return self.exec.update_by_id(user_id, data)

    def increment_losses(self, user_id: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
        now = datetime.now().isoformat()
        data = {'losses': user.get('losses', 0) + 1, 'updated_at': now}
        return self.exec.update_by_id(user_id, data)

    def increment_draws(self, user_id: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
        now = datetime.now().isoformat()
        data = {'draws': user.get('draws', 0) + 1, 'updated_at': now}
        return self.exec.update_by_id(user_id, data)
    ...
    def get_rank_list(self, limit: int = 50) -> List[Dict[str, Any]]:
        sql = f"SELECT * FROM {self.TABLE_NAME} WHERE status = 0 ORDER BY score DESC LIMIT {limit}"
