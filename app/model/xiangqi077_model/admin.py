...
class XiangqiAdminModel:
    TABLE_NAME = 'tb_xiangqi077_model_admin'
    ...
    def init_default_admin(cls):
        ...
        if not existing:
            model.create(username='admin', password='admin123', real_name='系统管理员')

    @staticmethod
    def _hash_password(password: str, salt: str) -> str:
        return hashlib.sha256((password + salt).encode()).hexdigest()

    def create(self, username: str, password: str, real_name: str = '') -> int:
    ...
    def update_password(self, admin_id: int, new_password: str) -> int:
        ...
        now = datetime.now().isoformat()
        data = {'password_hash': password_hash, 'salt': salt, 'updated_at': now}
