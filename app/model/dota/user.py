...
class DotaUserModel:
    ...
    def create_table(cls):
        ...
        sql = f"""
            CREATE TABLE IF NOT EXISTS {cls.TABLE_NAME} (
                ...
                current_hero_id INTEGER DEFAULT 0,
    ...
    def update_exp(self, user_id: int, exp_gain: int) -> Dict[str, Any]:
        user = self.get_by_id(user_id)
        if not user:
            return {'leveled_up': False, 'new_level': user.get('level', 1)}

        current_level = user.get('level', 1)
        current_exp = user.get('exp', 0)
        new_exp = current_exp + exp_gain

        exp_required = self.get_exp_required(current_level)
        leveled_up = False
        ...
        while new_exp >= exp_required and new_level < 30:
            ...
            new_level += 1
            leveled_up = True
            exp_required = self.get_exp_required(new_level)
    ...
    def get_exp_required(level: int) -> int:
        base_exp = 100
        increase = 50
        return base_exp + (level - 1) * increase + max(0, level - 3) * 5
