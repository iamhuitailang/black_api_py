...
class KuaidiUserModel:
    TABLE_NAME = 'tb_kuaidi_077_model_user'
    ...
    def init_default_admin(cls):
        db = get_db()
        check_sql = f"SELECT COUNT(*) as count FROM {cls.TABLE_NAME} WHERE role = ?"
        result = db.fetch_one(check_sql, (cls.ROLE_ADMIN,))
        ...
        insert_sql = f"""
            INSERT INTO {cls.TABLE_NAME} (phone, password_hash, salt, nickname, role, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
        db.execute(insert_sql, ('admin', password_hash, salt, '系统管理员', cls.ROLE_ADMIN, cls.STATUS_ACTIVE, now, now))

    @staticmethod
    def _hash_password(password: str, salt: str) -> str:
        return hashlib.sha256((password + salt).encode()).hexdigest()

    def create(self, phone: str, password: str, nickname: str = '', role: int = ROLE_USER) -> int:
        salt = secrets.token_hex(8)
