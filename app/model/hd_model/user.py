...
class UserModel:
    ...
    def create(self, username: str, password: str, nickname: str = '', avatar: str = '') -> int:
        ...
        data = {
            ...
            'salt': salt,
            'nickname': nickname or f'忍者{username[-4:]}',
            'avatar': avatar,
            'level': 1,
            'exp': 0,
            'chakra': 100,
    ...
    def verify_password(self, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        if password_hash == user.get('password_hash'):
            return {
                ...
                'exp': user.get('exp'),
                'chakra': user.get('chakra'),
                'max_chakra': user.get('max_chakra'),
                'gold': user.get('gold'),
    ...
    def update(self, user_id: int, data: Dict[str, Any]) -> int:
        ...
        update_data = {k: v for k, v in data.items() if k in [
            'nickname', 'avatar', 'level', 'exp', 'chakra', 'max_chakra', 'gold', 'status'
        ]}
    ...
    def update_chakra(self, user_id: int, delta: int) -> int:
        ...
        new_chakra = max(0, min(max_chakra, current_chakra + delta))
    ...
    def update_gold(self, user_id: int, delta: int) -> int:
        ...
        new_gold = max(0, current_gold + delta)
    ...
    def add_exp(self, user_id: int, exp_amount: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
    ...
    def get_all(self, page: int = 1, page_size: int = 10, status: int = None,
                level: int = None, keyword: str = None) -> Dict[str, Any]:
        conditions = {}
    ...
    def search(self, keyword: str, page: int = 1, page_size: int = 10,
               status: int = None, level: int = None) -> Dict[str, Any]:
