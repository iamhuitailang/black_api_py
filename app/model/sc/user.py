...
class ScUserModel:
    ...
    def update_experience(self, user_id: int, exp: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0
    ...
    def update_reputation(self, user_id: int, delta: int) -> int:
        user = self.get_by_id(user_id)
        if not user:
            return 0

        current_rep = user.get('reputation', 0)
